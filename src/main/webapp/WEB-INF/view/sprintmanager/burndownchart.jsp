...
<div class="wrap-content container" id="container">
	...
	<div class="container-fluid container-fullw bg-white">
		<div class="row">
			<div id="spentCharttext" class="center-block"></div>
		</div>
		<div id="burndown"></div>
	</div>
</div>
...
<script>
	...
	for (var i = 0; i < burnData.length; i++) {
		for (var j = 0; j < burnData[i].length; j++) {
			for (var k = 0; k < burnData[i][j].length; k++) {
				
				if(k==0){
					
					days[c++]=burnData[i][j][k];
				}
				if(k==1){
					console.log(burnData[i][j][k]);
					burnValues[ca++]=burnData[i][j][k];
				}
	...
	var html = '<table class="table table-bordered info">' + '<thead>' + '<tr>'
			+ ' <th>Sprint code</th>' + ' <th>Sprint Title</th>'
			+ ' <th>Sprint Goal</th>' + ' <th>Sprint start date</th>'
			+ ' <th>Sprint end date</th>' + '</tr>' + '</thead>' + '<tbody>'
			+ '<tr>' + ' <td>' + sp[0].sprintCode + '</td>' + ' <td>'
			+ sp[0].sprintName + '</td>' + ' <td>' + sp[0].sprintGoal + '</td>'
			+ ' <td>' + sp[0].startDate + '</td>' + ' <td>' + sp[0].endDate
			+ '</td>' + '</tr>' + '</tbody>' + ' </table>';
